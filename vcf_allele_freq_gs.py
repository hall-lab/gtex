    ...
    for line in vcf_file:
        if in_header:
            if line.startswith('##'):
                ...
                vcf.add_info('MODECN', '1', 'Integer', 'Mode copy number')
        ...
        # skip non-genome strip variants
        if not v[2].startswith("GS_"):
            print line.rstrip()
            continue
        ...
        # get format index of CN field
        fmt_map = v[8].split(":")
        cn_index = fmt_map.index('CN')

        gt_list = []
        for i in xrange(9,len(v)):
            gt_string = v[i].split(':')[cn_index]

            if '.' in  gt_string:
                continue
            gt_list.append(int(gt_string))

        # print gt_list
        gt_mode = max(stats.mode(gt_list)[0])
        num_non_ref = sum([ x != gt_mode for x in gt_list ])
        allele_freq = float(num_non_ref) / len(gt_list)

        # print  'nonref', num_non_ref
        # print float(num_non_ref) / len(gt_list)
        # print 'bal', sum([ abs(x - gt_mode) for x in gt_list ]) / len(gt_list)

        # populate AF and NSAMP
        var.info['AF'] = '%.4g' % allele_freq
        var.info['NSAMP'] = num_non_ref
        var.info['MODECN'] = gt_mode
