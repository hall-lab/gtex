...
cat tissues/$TISSUE/$EGENE/$EGENE.nom.eqtl.txt \
    | awk -v PTHRES=$PTHRES -v EGENE=$EGENE '{ if ($1==EGENE && $2~"b37" && $4!~"nan") print $2,$5 }' OFS="\t" \
...
ESV=`cat tissues/$TISSUE/$EGENE/$EGENE.nom.eqtl.txt | awk '$4!~"nan$"' | awk '$2!~"_b37$"' | sort -k 5,5g | awk '{ print $2; exit }'`
